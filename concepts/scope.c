/*
Non compileable code which illustrates the concept of scoping variables, nested scopes,
and how identifiers defined in an inner scope hide the identifiers defined in the outer scope
*/
...
void f(int i) // block scope of i begins
{
    ...
    for (int i = 0; i < 2; i++) // block scope of the loop-local i begins
    {
